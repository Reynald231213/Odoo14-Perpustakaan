...
class katu_perpustakaan(models.Model):
    ...
    kartu_lines = fields.One2many(comodel_name='kartu_perpustakaan_line', inverse_name='kartu_id', string='kartu perpustakaan line', readonly=True, states={'confirm': [('readonly', False)]})
...
class kartu_perpustakaan_line(models.Model):
    ...
    kartu_id = fields.Many2one(comodel_name='kartu_perpustakaan', string='Kartu Perpustakaan', required=True, ondelete='cascade')
    ...
    anggota_id = fields.Many2one(comodel_name='anggota', string='Member', related="kartu_id.anggota_id", store=True, readonly=True)
